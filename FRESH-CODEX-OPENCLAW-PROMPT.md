...
```text
Склонируй https://github.com/NickStr11/nickCodex-READY.git в удобную папку на этом ноуте, открой repo как workspace, прочитай AGENTS.md, README.md, PORTABILITY.md, OPENCLAW-SECOND-LAPTOP.md и scripts/openclaw-second-laptop.config.psd1, потом запусти .\setup-openclaw-laptop.ps1.
...
Цель: подготовить этот ноут под отдельный Codex/OpenClaw аккаунт с моделью из scripts/openclaw-second-laptop.config.psd1. Если native Windows путь начнет ломаться, не закапывайся, а предложи переход OpenClaw в WSL2 по WSL-MIGRATION.md.
```
